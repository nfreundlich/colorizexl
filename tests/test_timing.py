...
for factor in np.arange(0.1, 1.1, step=0.1):
    # scaled filenames
    input_grayscale_name = "./output_timing/girl_grayscale_" + str(factor) + ".png"
    input_annotation_name = "./output_timing/girl_annotation_" + str(factor) + ".png"
    ground_truth_small_name = "./output_timing/girl_gt_" + str(factor) + ".png"
    ...
    # save output image
    output_name = "./output_timing/out_" + str(factor) + ".png"
    cv2.imwrite(
        output_name, cv2.cvtColor(output.astype("float32"), cv2.COLOR_BGR2RGB) * 255.0,
    )

    # metrics
    ssim, mse, pnsr = colorizer.compare_images_metrics(img=output, gt=im_gt_small)

    df = pd.DataFrame([[factor, output_name, elapsed, ssim, mse, pnsr]])
...
df_all.to_csv("./output_timing/results_timing.csv", index=False)
